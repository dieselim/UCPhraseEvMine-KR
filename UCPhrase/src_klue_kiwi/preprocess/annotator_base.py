...
class BaseAnnotator:
    def __init__(self, preprocessor: Preprocessor, use_cache=True):
        ...
        self.path_tokenized_pos_corpus = self.preprocessor.path_tokenized_pos_corpus # POS 정보 파일 경로 추가
    ...
    def _par_sample_train_data(marked_doc):
        ...
        for sent in sents:
            ...
            # 집합을 리스트로 변환
            possible_negative_spans = list(possible_negative_spans)
    ...
    def sample_train_data(self):
        ...
        sampled_docs = [BaseAnnotator._par_sample_train_data(d) for d in tqdm(marked_docs, ncols=100, desc='[Sample Train]', dynamic_ncols=True)]
