...
class FeatureExtractor(BaseFeatureExtractor):
    ...
    def _get_model_outputs(self, marked_sents):
        ...
        with torch.no_grad():
            for input_ids_batch, input_masks_batch, _ in tqdm(input_idmasks_spans_batches, ncols=100, desc='inference', dynamic_ncols=True):
    ...
    def generate_train_instances(self, path_sampled_docs, max_num_docs=None):
        ...
        train_instances = []
        for i, model_output_dict in tqdm(enumerate(model_outputs), ncols=100, total=len(marked_sents), desc='[Feature] Generate train instances', dynamic_ncols=True):
        ...
        utils.Pickle.batch_dump(train_instances, path_output)
        return path_output
    ...
    def generate_predict_docs(self, path_marked_corpus, max_num_docs=None):
        ...
        predict_instances = []
        for i, model_output_dict in tqdm(enumerate(model_outputs), ncols=100, total=len(marked_sents), desc='Generate predict instances', dynamic_ncols=True):
            marked_sent = marked_sents[i]
            word_idxs = marked_sent['widxs']
        ...
        utils.Pickle.batch_dump(predict_docs, path_output)
