...
class AttmapTrainLoader:
    ...
    def load_train_data(self, filepath, sample_ratio=-1):
        ...
        instances = utils.Pickle.load_all_data(filepath)
...
class AttmapTrainer:
    ...
    def train(self, path_sampled_train_data, num_epochs=20):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            self.model.train()
            epoch_loss = 0
            for attmap_features, gtlabels in tqdm(train_data, total=num_train, ncols=100, dynamic_ncols=True):
            ...
            pred_labels = []
            with torch.no_grad():
                for attmap_features, gtlabels in tqdm(valid_data, total=num_valid, ncols=100, dynamic_ncols=True):
                    attmap_features = attmap_features.to(DEVICE)
            ...
            all_valid_f1_scores.append(valid_f1)
        ...
        # After all epochs are done, find the best epoch
        best_epoch = max(range(num_epochs), key=lambda i: all_valid_f1_scores[i])
        best_valid_f1 = all_valid_f1_scores[best_epoch]
        utils.Log.info(f'Training complete. Best epoch: {best_epoch + 1}, Best valid f1: {best_valid_f1}')

        return best_epoch + 1  # Return 1-based epoch index
