...
class BaseModel(nn.Module):
    ...
    @staticmethod
    def _par_decode_doc(predicted_doc, pos_doc, threshold):
        sents = []
        for predicted_sent, pos_sent in zip(predicted_doc['sents'], pos_doc['sents']):
            assert len(predicted_sent['ids']) == len(pos_sent), \
            f"Length mismatch: predicted_sent length = {len(predicted_sent['ids'])}, pos_sent length = {len(pos_sent)}"
            ...
            for l_idx, r_idx, prob in predicted_spans:
                ...
                if idxs_set & idxs_taken:
                    continue
                if not tokens[l_idx].startswith(consts.GPT_TOKEN):
                    if (r_idx + 1 < len(tokens) and not tokens[r_idx + 1].startswith(consts.GPT_TOKEN)) or r_idx == len(tokens):
                        if pos_sent[l_idx] in ['NNG', 'NNP']:
                            idxs_taken |= idxs_set
    ...
    @staticmethod
    def decode(path_predicted_docs, path_pos_docs, output_dir, threshold, use_cache, use_tqdm):
        ...
        path_pos_docs = Path(path_pos_docs)
        pos_docs = utils.JsonLine.load(path_pos_docs)

        # _id_ 기준으로 정렬
        predicted_docs = sorted(predicted_docs, key=lambda doc: doc['_id_'])
        pos_docs = sorted(pos_docs, key=lambda doc: doc['_id_'])
        
        if use_tqdm:
            doc2sents = {doc['_id_']: BaseModel._par_decode_doc(doc, pos_doc, threshold) for doc, pos_doc in tqdm(zip(predicted_docs, pos_docs), ncols=100, desc='Decode', total=len(predicted_docs), dynamic_ncols=True)}
        else:
            doc2sents = {doc['_id_']: BaseModel._par_decode_doc(doc, pos_doc, threshold) for doc, pos_doc in zip(predicted_docs, pos_docs)}
    ...
    def get_doc2cands(path_predicted_docs, output_dir, expected_num_cands_per_doc, use_cache, use_tqdm):
        ...
        path_predicted_docs = Path(path_predicted_docs)
        predicted_docs = utils.Pickle.load(path_predicted_docs)
        to_iterate = tqdm(predicted_docs, ncols=100, desc='Decode', dynamic_ncols=True) if use_tqdm else predicted_docs
        threshold_l = 0.0
        threshold_r = 1.0
