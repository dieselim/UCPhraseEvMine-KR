...
def get_sentences(doc_sents, tokenizer):
    ...
    masked_sentences = []
    emb_index = []
    for di, doc in tqdm(enumerate(doc_sents), dynamic_ncols=True, total=len(doc_sents)):
...
def get_pretrained_emb(model, tokenizer, sentences, np_file, dim=768, batch_size=128):
    ...
    this_idx = 0
    for i in tqdm(range(iterations), dynamic_ncols=True):
        start = i * batch_size
        end = min((i+1)*batch_size, len(sentences))
        
        max_length = 512
        batch_ids = []
        for ids, _, _ in sentences[start:end]:
...
def get_phrase_emb(args, model_name = 'klue/bert-base'):
